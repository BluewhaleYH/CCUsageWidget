...
/** 표시 에이전트 수 n에 맞는 **그리드(패널) 콘텐츠 너비**(px, chrome 제외). 캐러셀 패널 폭. */
export function gridWidth(n: number): number {
  // 그리드 아이템 = 라벨 + 에이전트 n개 → 사이 간격 n개
  return LABEL_WIDTH + n * AGENT_WIDTH + n * COLUMN_GAP
}

/** 표시 에이전트 수 n에 맞는 **창 너비**(px, chrome 포함). */
export function contentWidth(n: number): number {
  return gridWidth(n) + CHROME_WIDTH
}
