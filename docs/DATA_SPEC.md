...
```
 ...
 │              │   일일    │   월간    │   ← 열 = 기간
 ├──────────────┴──────────┴──────────┤
 │ Claude  [모델칩…]                   │   ← 프로바이더 그룹 헤더
 │ Cost          │ $135.90  │ $166.25  │
 │ Input         │ 25.5K    │ 47.7K    │
 │ Output        │ 490.1K   │ 698.5K   │
 │ Cache Create  │ 1.6M     │ 2.2M     │
 │ Cache Read    │ 214.6M   │ 253.6M   │
 │ Total         │ 216.8M   │ 256.5M   │
 ├─────────────────────────────────────┤
 │ Codex   없음   │  Gemini  없음        │   ← 데이터 없으면 "없음"
 └─────────────────────────────────────┘
```
- **레이아웃(확정)**: **단일 표**. 열 = 기간(일일/월간), **프로바이더(Claude/Codex/Gemini) 그룹**마다
  그룹 헤더(이름 + **모델 칩**) + 지표 행(Cost / Input / Output / Cache Create / Cache Read / **Total**).
- 토큰 수는 **K/M/B 축약**(우측 정렬·tabular-nums), **전체 숫자는 hover 툴팁**. 데이터 없는 셀은 `—`, 그룹 전체 없으면 `없음`.
- 넘치면 데이터 영역 **세로 스크롤**(기간 헤더 sticky).
