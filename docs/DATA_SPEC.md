...
```
  ...
  ─ 월간 ───────────────   (동일)
```
- **레이아웃(확정)**: 기간(일일/월간) **섹션**마다 프로바이더 **카드 3개**(Claude/Codex/Gemini, 대문자 라벨).
- **각 카드 표시(ccusage 전체 항목)**: 비용($) 메인 + **모델 칩**(여러 개면 줄바꿈) +
  토큰 세부(Input / Output / Cache Create / Cache Read / **Total**).
- 토큰 수는 **K/M/B 축약**으로 표시하고, **전체 숫자는 hover 툴팁**으로 제공.
- 항목이 많아 데이터 영역은 **내용 높이에 맞추고, 넘치면 세로 스크롤**한다.
