...
- [x] 컨트롤 헤더바: ✕ 종료 / ─ 최소화(접기) / □ 최대화(상세 확장) — 배치·hover·no-drag·뷰활성·연결점(connDot). 검수: typecheck/build + 스모크 ✓ (이슈 #55)
- [x] 최소화=접기(헤더만), 최대화=상세 확장 동작 + 상태 영속화 — widget:setView/getView + store view, 확장 시 input/output 분리. 검수: typecheck/build + viewHeight 스모크 ✓ (이슈 #57)
