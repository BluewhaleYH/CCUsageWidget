...
- **데이터 출처**: **위젯이 실행되는 로컬 컴퓨터(내장 `local` 호스트, 기본 선택)** + **SSH로 등록된 원격 호스트**에서
  실행한 `ccusage` CLI (`--json` 출력 파싱). `local`은 SSH 없이 로컬 실행, 원격은 SSH 실행.
- **멀티 호스트**: 로컬 + 등록한 원격들을 좌/우 전환으로 하나씩 조회 (현재 선택 호스트만 폴링)
